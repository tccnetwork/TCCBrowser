...
//!    ký, tức là gói không bị sửa. Ai ký thì chưa biết. Có phép thử ở `loi.rs`.
...
pub const HANH_DONG_CHO_PHEP: &str = "cho-phep";
pub const HANH_DONG_TU_CHOI: &str = "tu-choi";
...
#[must_use]
pub fn ma_cong_tac(ten_quyen: &str) -> String {
    format!("q-{ten_quyen}")
}
...
#[must_use]
pub fn quyet_dinh(hanh_dong: Option<&str>, dang_bat: &[String], ten_quyen: &str) -> Decision {
    if hanh_dong != Some(HANH_DONG_CHO_PHEP) {
        return Decision::Deny;
    }
    if dang_bat.contains(&ma_cong_tac(ten_quyen)) {
        Decision::Allow
    } else {
        Decision::Deny
    }
...
pub fn dung(m: &Manifest, ngon_ngu: NgonNgu) -> Result<Node, UiError> {
    dung_voi_nguoi_ky(m, ngon_ngu, &TinhTrangNguoiKy::LanDau)
}
...
pub fn dung_voi_nguoi_ky(
    ...
    let mut man = Node::group(Flow::Column, Gap::Large)
        .child(Node::text_with(t(Khoa::QuyenTieuDe), Emphasis::Title)?)?
        ...
        .child(Node::text_with(t(Khoa::NguonKhongRo), Emphasis::Subtle)?)?;
    ...
    if let TinhTrangNguoiKy::DoiKhoa { van_tay_cu } = nguoi_ky {
        man = man
            .child(Node::text_with(t(Khoa::DoiKhoaKy), Emphasis::Title)?)?
            .child(Node::text_with(
                t(Khoa::DoiKhoaKyGiaiThich),
                Emphasis::Normal,
            )?)?
            .child(Node::text_with(
                format!("{}: {van_tay_cu}", t(Khoa::KhoaCu)),
                Emphasis::Subtle,
            )?)?;
    }

    if m.capabilities.is_empty() {
        man = man.child(Node::text(t(Khoa::QuyenKhongXinGi))?)?;
    } else {
    ...
    man.child(Node::text_with(
        t(Khoa::QuyenCanhBaoDanhTinh),
        Emphasis::Subtle,
    )?)?
    .child(
        Node::group(Flow::Row, Gap::Medium)
            .child(Node::button(
                t(Khoa::QuyenNutTuChoi),
                "tu-choi",
                Tone::Neutral,
            )?)?
...
fn muc_quyen(c: &CapabilityRequest, ngon_ngu: NgonNgu) -> Result<Node, UiError> {
    let t = |k: Khoa| nhan(k, ngon_ngu);

    let (tieu_de, chi_tiet) = match &c.scope {
        Scope::Network { hosts } => (t(Khoa::QuyenMang), hosts.join(", ")),
        Scope::Storage { quota_bytes } => {
            (t(Khoa::QuyenLuuTru), format!("{} KB", quota_bytes / 1024))
        }
        Scope::Wallet {
            ...
            if *may_request_signature {
                t(Khoa::ViDuocXinChuKy)
            } else {
                t(Khoa::ViChiDocDiaChi)
            }
            .to_owned(),
        ),
    };
    ...
    Node::group(Flow::Column, Gap::Small)
        ...
        .child(Node::toggle(tieu_de, false, &ma_cong_tac(&c.name))?)?
...
"content_hash":"{}","entry":"index.html","capabilities":{quyen_json}}}"#,
    ...
    fn chu_tren_man_hinh(m: &Manifest, n: NgonNgu) -> String {
        let mut s = String::new();
        moi_nhan(&dung(m, n).unwrap().accessibility_tree(), &mut s);
        s
    }
    ...
    fn hien_ten_ung_dung_va_pham_vi_cu_the() {
        let s = chu_tren_man_hinh(&ke_khai(XIN_MANG), NgonNgu::En);
    ...
    fn doi_khoa_ky_thi_canh_bao_hien_ra_truoc_danh_sach_quyen() {
        ...
        let tt = TinhTrangNguoiKy::DoiKhoa {
            van_tay_cu: "aaaaaaaaaa…bbbbbbbbbb".to_owned(),
        };
        let cay = dung_voi_nguoi_ky(&m, NgonNgu::En, &tt).unwrap();
        let mut s = String::new();
        moi_nhan(&cay.accessibility_tree(), &mut s);

        let canh_bao = nhan(Khoa::DoiKhoaKy, NgonNgu::En);
        ...
        let vt_quyen = s
            .find(nhan(Khoa::QuyenVi, NgonNgu::En))
            .expect("có mục quyền");
        assert!(
    ...
    fn khong_doi_khoa_thi_khong_canh_bao() {
        for tt in [TinhTrangNguoiKy::LanDau, TinhTrangNguoiKy::KhopKhoaCu] {
            let cay = dung_voi_nguoi_ky(&ke_khai(XIN_VI_KY), NgonNgu::En, &tt).unwrap();
            let mut s = String::new();
            moi_nhan(&cay.accessibility_tree(), &mut s);
            assert!(
                !s.contains(nhan(Khoa::DoiKhoaKy, NgonNgu::En)),
    ...
    fn luon_co_canh_bao_danh_tinh() {
        for q in ["[]", XIN_MANG, XIN_VI_KY] {
            for n in [NgonNgu::En, NgonNgu::Vi] {
                ...
                assert!(
                    s.contains(nhan(Khoa::QuyenCanhBaoDanhTinh, n)),
    ...
    fn khong_xin_gi_thi_noi_ro_la_khong_xin_gi() {
        let s = chu_tren_man_hinh(&ke_khai("[]"), NgonNgu::En);
        assert!(s.contains(nhan(Khoa::QuyenKhongXinGi, NgonNgu::En)), "{s}");
    }
    ...
    fn quyen_ky_giao_dich_noi_ro_la_chuyen_tien() {
        let s = chu_tren_man_hinh(&ke_khai(XIN_VI_KY), NgonNgu::En);
        assert!(s.contains("this moves money"), "{s}");

        let sv = chu_tren_man_hinh(&ke_khai(XIN_VI_KY), NgonNgu::Vi);
    ...
    fn vi_chi_doc_khong_bi_ghi_nham_thanh_ky_duoc() {
        let chi_doc = r#"[{"name":"wallet",
            ...
            "reason":"hiện số dư"}]"#;
        let s = chu_tren_man_hinh(&ke_khai(chi_doc), NgonNgu::En);
    ...
    fn moi_duong_khong_ro_rang_deu_ra_tu_choi() {
        ...
        assert_eq!(
            quyet_dinh(Some(HANH_DONG_CHO_PHEP), &du, "network"),
            Decision::Allow
        );

        for x in [
            None,                    // đóng cửa sổ, không bấm gì
            Some(HANH_DONG_TU_CHOI), // bấm từ chối
            Some(""),                // chuỗi rỗng
    ...
    fn bam_cho_phep_ma_khong_bat_gi_thi_khong_cap_quyen_nao() {
        for q in ["network", "storage", "wallet"] {
            assert_eq!(
                quyet_dinh(Some(HANH_DONG_CHO_PHEP), &[], q),
                Decision::Deny,
    ...
    fn bat_mot_quyen_khong_keo_theo_quyen_khac() {
        ...
        assert_eq!(
            quyet_dinh(Some(HANH_DONG_CHO_PHEP), &chi_mang, "network"),
            Decision::Allow
        );
        assert_eq!(
            quyet_dinh(Some(HANH_DONG_CHO_PHEP), &chi_mang, "wallet"),
            Decision::Deny,
    ...
    fn ma_cong_tac_khop_chinh_xac() {
        for gia in [
            ...
            assert_eq!(
                quyet_dinh(Some(HANH_DONG_CHO_PHEP), &bat(&[gia]), "network"),
                Decision::Deny,
    ...
    fn ma_cong_tac_khong_dung_do_ma_nut() {
        assert_ne!(ma_cong_tac("cho-phep"), HANH_DONG_CHO_PHEP);
        assert_ne!(ma_cong_tac("tu-choi"), HANH_DONG_TU_CHOI);
    }
    ...
    fn hai_ma_hanh_dong_that_su_nam_tren_cay() {
        let cay = dung(&ke_khai(XIN_VI_KY), NgonNgu::En).unwrap();
        let ds: Vec<&str> = cay.action_ids().iter().map(|a| a.as_str()).collect();
        assert!(
            ds.contains(&HANH_DONG_CHO_PHEP),
            "hằng số cho phép \"{HANH_DONG_CHO_PHEP}\" không có trên cây: {ds:?}"
        );
        assert!(
            ds.contains(&HANH_DONG_TU_CHOI),
            "hằng số từ chối \"{HANH_DONG_TU_CHOI}\" không có trên cây: {ds:?}"
        );
    ...
    fn moi_cong_tac_mac_dinh_tat() {
        ...
        let cay = dung(&ke_khai(nhieu), NgonNgu::En).unwrap();
        let mut ds = Vec::new();
    ...
    fn xin_nhieu_quyen_van_dung_duoc_hop_thoai() {
        ...
        let cay = dung(&m, NgonNgu::En).expect("hộp thoại hỏng khi xin nhiều quyền");
